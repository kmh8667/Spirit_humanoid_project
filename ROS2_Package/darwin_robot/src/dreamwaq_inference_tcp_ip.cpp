...
public:
    InferenceService(): Node("infernce_service"){
        ...
        this->declare_parameter("num_history_steps", 5);
        ...
        RCLCPP_INFO(this->get_logger(), "Looking for python client");
        client_fd = accept(server_fd, nullptr, nullptr);
        RCLCPP_INFO(this->get_logger(), "Service ready");

        timer_ = this->create_wall_timer(20ms, std::bind(&InferenceService::process, this));

    }

private:
    void process(){
        const size_t in_size = 63;
        const size_t out_size = 18;
        const size_t in_bytes = in_size * sizeof(float);
        const size_t out_bytes = out_size * sizeof(float);

        std::vector<float> obs(63);
        ssize_t recvd = recv(client_fd, obs.data(), in_bytes, MSG_WAITALL);
        if (recvd <= 0) {
            RCLCPP_ERROR(this->get_logger(), "Client disconnected or failed to send");
            return;
        }
        ...
        // Convert obs from float to double for consistency
        std::vector<double> obs_double(obs.begin(), obs.end());
        
        torch::Tensor obs_tensor = torch::tensor(obs_double, torch::TensorOptions().dtype(torch::kFloat32).device(device_));
        ...
        input.push_back(obs_history_tensor.unsqueeze(0));
        ...
        // Convert back to float for TCP sending (maintain compatibility)
        std::vector<float> raw_actions_float(raw_actions.begin(), raw_actions.end());
            
        ssize_t sent = send(client_fd, raw_actions_float.data(), out_bytes, 0);
        
        // Publish joint commands for openRB_joint_cmd.py
        auto joint_state = sensor_msgs::msg::JointState();
        joint_state.position = raw_actions;
        pub_raw_joint_cmd_->publish(joint_state);
        
        if (debug_) {
            RCLCPP_INFO_STREAM(this->get_logger(), "Action: " << raw_actions.size() << " values published");
        }
...
int main(int argc, char* argv[]){
    ...
    auto node = std::make_shared<InferenceService>();
    rclcpp::spin(node);
